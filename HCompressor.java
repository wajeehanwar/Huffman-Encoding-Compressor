...
// int getUniqueChars()                 --> Return # of unique ch
// void setUniqueChars(uniqueCharsByte) --> Set # of unique ch
// int fileLength()                     --> Return length of file
// void setFileLength(length)           --> Set length of file
// ****************************ERRORS**********************************
// No error checks.
class CharCounter {
	public CharCounter() {
	...
	public void setUniqueChars(byte uniqueCharsByte) {
		uniqueChars = uniqueCharsByte & 0xff;
	}
...
class HuffmanTree {
	...
	public void writeEncodingTable(DataOutputStream out) throws IOException {
		out.writeInt(theCounts.getFileLength());
		out.writeByte((byte)theCounts.getUniqueChars());
	...
	{
		theCounts.setFileLength(in.readInt());
		theCounts.setUniqueChars(in.readByte());
		for (int i = 0; i < BitUtils.DIFF_BYTES; i++) {
...
class HZIPInputStream extends InputStream {
...
class HZIPOutputStream extends OutputStream {
	...
	public void close() throws IOException {
		...
		byteIn.close();
		HuffmanTree huffmanTreeCodes = new HuffmanTree(countObj);
		huffmanTreeCodes.writeEncodingTable(dout);
		BitOutputStream bout = new BitOutputStream(dout);
		for (int i = 0; i < theInput.length; i++) {
			bout.writeBits(huffmanTreeCodes.getCode(theInput[i] & (0xff)));
		}
		bout.writeBits(huffmanTreeCodes.getCode(BitUtils.EOF));
		bout.close();
		byteOut.close();
	}
...
public class HCompressor {
	...
	public static void uncompress(String compressedFile) throws IOException {
		...
		// for debugging, so as to not clobber original
		if (inFile.indexOf(".") > 0) {
